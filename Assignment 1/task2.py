...
Element* search (char* name){
    unsigned long int index = hash_function(name);
    int i;
    for(i=0;i<ARRAY_SIZE;i++){
...
bool insert(Element* p){
    ...
    unsigned long int index = hash_function(p->name);
...
void print_table(void){
    ...
    for(i=0;i<ARRAY_SIZE;i++){
        if(hash_table[i] == NULL){
            ...
            printf("\t%i\t%s\t%d\n",i,hash_table[i]->name,hash_table[i]->freq);
        }
    }
...
{
    init_hash_table();
    load_file("/Users/narendersinghroha/Downloads/names.csv");
